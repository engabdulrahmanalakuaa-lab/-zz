// ==========================================
// renderer.js - تقنيات سوفت (نسخة معدلة وآمنة بالكامل)
...
// ============ النافذة المنبثقة المخصصة (بديل لـ prompt) ============
function customPrompt(message, type = 'text', defaultVal = '') {
    return new Promise((resolve) => {
        const modal = document.getElementById('modal');
        const content = document.getElementById('modal-content');
        content.innerHTML = `
            <div style="text-align:center;">
                <h3 style="margin-bottom:15px; color:var(--text);">${message}</h3>
                <input type="${type}" id="prompt-input" value="${defaultVal}" class="form-group" style="width:100%; padding:12px; border:2px solid #bdc3c7; border-radius:6px; margin-bottom:15px; font-size:16px;">
                <div>
                    <button id="prompt-ok" class="btn btn-primary" style="margin-left:10px; width:100px;">حسناً</button>
                    <button id="prompt-cancel" class="btn btn-secondary" style="width:100px;">إلغاء</button>
                </div>
            </div>
        `;
...
window.addEventListener('DOMContentLoaded', async () => {
    // تفعيل أزرار التنقل فوراً لتجنب أي أعطال لاحقة
...
async function showLoginScreen() {
    ...
    if (companies.length === 0) {
        const name = await customPrompt('أدخل اسم المطعم (سيظهر في الفواتير):', 'text', 'مطعم كيان الشواية البخاري');
        if (!name) return;
        ...
        const chosen = await new Promise(resolve => {
            document.getElementById('modal-content').innerHTML = `
                <div style="text-align:center;">
                    <h3 style="margin-bottom:15px;">اختر المطعم</h3>
                    <select id="choose-company-select" class="form-group" style="width:100%; padding:10px; margin-bottom:15px;">${options}</select>
                    <button id="choose-ok" class="btn btn-primary" style="width:100%;">متابعة</button>
                </div>`;
            document.getElementById('modal').classList.add('active');
            document.getElementById('choose-ok').onclick = () => {
                const val = document.getElementById('choose-company-select').value;
...
async function submitLogin() {
    ...
    const shiftOpened = await openShiftIfNeeded();
    if (!shiftOpened) return; // توقف إذا تم إلغاء فتح الوردية
...
async function openShiftIfNeeded() {
    ...
    if (!shiftData) {
        const val = await customPrompt('أدخل رصيد افتتاح الصندوق (ر.س):', 'number');
        if (val === null) return false; // تم الإلغاء
        const openingCash = parseFloat(val) || 0;
...
async function renderDashboard() {
    ...
    // رسم بياني محلي HTML 100% بدلاً من الاعتماد على Chart.js المسبب للأعطال بدون إنترنت
    const chartHtml = `
    <div style="background:var(--card); padding:15px; border-radius:10px; margin-bottom:20px; box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        <h3 style="margin-bottom:15px;">مبيعات آخر 7 أيام</h3>
        <div style="display:flex; justify-content:space-around; align-items:flex-end; height:180px; border-bottom:1px solid #ddd; padding-bottom:5px;">
            ${last7.map((sum, i) => {
                let max = Math.max(...last7) || 1;
                let height = (sum / max) * 100;
                let labels = ['قبل 6', 'قبل 5', 'قبل 4', 'قبل 3', 'قبل 2', 'أمس', 'اليوم'];
                return `<div style="display:flex; flex-direction:column; align-items:center; width:45px; height:100%;">
                    <div style="flex-grow:1; display:flex; flex-direction:column; justify-content:flex-end; width:100%;">
                        <span style="font-size:11px; text-align:center; color:#7f8c8d; margin-bottom:4px;">${sum.toFixed(0)}</span>
                        <div style="width:100%; height:${height}%; background:var(--primary-light); border-radius:4px 4px 0 0; min-height:4px;"></div>
                    </div>
                    <span style="font-size:11px; margin-top:5px; font-weight:bold; color:var(--text);">${labels[i]}</span>
                </div>`;
            }).join('')}
        </div>
    </div>`;
    ...
    document.getElementById('main-content').innerHTML = `
        ...
        <div>
            <h3>آخر الطلبات</h3>
            <table><thead><tr><th>رقم</th><th>المبلغ (ر.س)</th><th>التاريخ</th></tr></thead><tbody>
...
async function closeShift() {
    const val = await customPrompt('أدخل النقد الفعلي بالدرج (ر.س):', 'number');
    if (val === null) return;
    ...
    <html><body style="font-family: Tajawal; direction: rtl; width: 74mm; padding: 5mm;">
        ...
        <p>مبيعات مسجلة: ${totalSalesCash.toFixed(2)} ر.س</p>
        <p>نقد فعلي: ${actual.toFixed(2)} ر.س</p>
...
async function renderProducts() {
    ...
    document.getElementById('main-content').innerHTML = `
        ...
        <table><tr><th>صورة</th><th>الاسم</th><th>الفئة</th><th>السعر (ر.س)</th><th>الوصفة</th><th></th></tr>${rows.join('')}</table>
    `;
}
...
async function renderMaterials() {
    ...
    document.getElementById('main-content').innerHTML = `
        ...
        <table><tr><th>الاسم</th><th>الوحدة</th><th>المخزون</th><th>الحد الأدنى</th><th>سعر الشراء (ر.س)</th><th></th></tr>
...
async function supplyMaterial(id) {
    const val = await customPrompt('كمية التوريد:', 'number');
    if (val === null) return;
...
async function generateReport() {
    ...
    document.getElementById('report-result').innerHTML = `
        ...
        <table><tr><th>رقم</th><th>المبلغ (ر.س)</th><th>التاريخ</th></tr>
...
async function openExpensesModal() {
    ...
    document.getElementById('modal-content').innerHTML = `
        ...
        <div class="form-group"><input type="number" id="exp-amount" placeholder="المبلغ (ر.س)" step="0.01"></div>
